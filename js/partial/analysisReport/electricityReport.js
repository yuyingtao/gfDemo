...
var electricityReportRender = {
    ...
    initTime: function initTime(_this) {
        ...
        $('#dimenTime').unbind().on('click', function () {
            ...
            switch (limit) {
                case "month":
                    WdatePicker({ el: "dimenTime", minDate: limitTime, dateFmt: 'yyyy/M', isShowClear: false, maxDate: '%y-%M' });break;
                case "year":
                    WdatePicker({ el: "dimenTime", minDate: limitTime, dateFmt: 'yyyy', isShowClear: false, maxDate: '%y' });break;
            }
        });
    },
    //切换年月维度
    changeDim: function changeDim(_this) {
        var that = this;
        $('.choose-m-y span').unbind().on('click', function () {
            ...
            switch (limit) {
                ...
                case "month":
                    $('#dimenTime').val(year + '/' + month);break;
                case "year":
                    $('#dimenTime').val(year);break;
            }
            var range = $('#dimenTime').val();
    ...
    //默认获取用户第一个电站名称及Id
    getPlant: function getPlant(_this) {
    ...
    //左边搜索框模糊搜索电站名称
    getPlants: function getPlants(_this, plantName) {
        ...
        $.http.POST('/report/fuzzySearchPlant.do', { tokenId: Cookies.getCook('tokenId'), plantName: plantName, storedPlantOnly: 1, plantType: Cookies.getCook('plantType') }, function (res) {
            $('.plantContain').hide();
            if (res.body.length) {
                var pEl = res.body.map(function (item) {
    ...
    //获取表格数据
    runDatatable: function runDatatable(_this, obj) {
        $('div.plantInfo').show();
        ...
        $.http.POST('/report/electricReportTable.do', obj, function (res) {
            ...
            $('div.plantInfo span').eq(1).text(plantName).end().eq(3).text(capacity).end().eq(5).text($("#dimenTime").val());
            _this.reportTable = $("#reportTable10").dataTable({
                ...
                columns: [{ data: "time", title: "时间" }, { data: "electriNumUsed", title: "用电量" }, { data: "electriNumOff", title: "下网电量" }, { data: "electriNumOn", title: "上网电量" }, { data: "electriMoneyOff", title: "下网费用" }, { data: "electriMoneyOn", title: "上网费用" }, { data: "electriMoneyUsed", title: "用电电费" }],
    ...
    //导出报表
    exportRep: function exportRep(_this) {
        var dimension = $('.activeSpan').data('dimension');
