...
var powerGenCurve = {
    ...
    //年月日切换 时间搜索数据
    getDMY: function getDMY(_this, type) {
        if (type == 1) {
    ...
    //获取限制时间
    getLimitDate: function getLimitDate(_this) {
        //限制时间
    ...
    //初始化时间控件
    initTime: function initTime(_this) {
        $('#dateStart').on('click', function () {
            var txt = $('.activeTime').text();
            var limit = _this.limitDate;
            switch (txt) {
                case '日':
                    WdatePicker({ el: "dateStart", readOnly: true, dateFmt: 'yyyy-MM-dd', minDate: '#F{$dp.$D(\'dateEnd\',{d:-30})}', maxDate: '#F{$dp.$D(\'dateEnd\',{d:-14})||$dp.$DV(\'%y-%M-%d\',{d:-14})}', isShowClear: false });break;
                case '月':
                    WdatePicker({ el: "dateStart", readOnly: true, dateFmt: 'yyyy-MM', minDate: limit, maxDate: '#F{$dp.$D(\'dateEnd\',{M:-1});}' && '%y-%M', isShowClear: false });break;
                case '年':
                    WdatePicker({ el: "dateStart", readOnly: true, dateFmt: 'yyyy', minDate: limit, maxDate: '%y', isShowClear: false });break;
            }
        });
        $('#dateEnd').on('click', function () {
            ...
            switch (txt) {
                case '日':
                    WdatePicker({ el: "dateEnd", readOnly: true, dateFmt: 'yyyy-MM-dd', minDate: '#F{$dp.$D(\'dateStart\',{d:14});}', maxDate: "" + (flag ? "%y-%M-%d" : "#F{$dp.$D(\'dateStart\',{d:30})}"), isShowClear: false });break;
                case '月':
                    WdatePicker({ el: "dateEnd", readOnly: true, dateFmt: 'yyyy-MM', minDate: '#F{$dp.$D(\'dateStart\',{M:1});}' || limit, maxDate: '%y-%M', isShowClear: false });break;
                case '年':
                    WdatePicker({ el: "dateEnd", dateFmt: 'yyyy', minDate: limit, maxDate: '%y', isShowClear: false, readOnly: true });break;
            }
        });
    },
    //发电量收益曲线
    powerProfit: function powerProfit(datas) {
    ...
    //电量收益曲线
    electricityProfit: function electricityProfit(datas) {
        if (JSON.stringify(datas) == "{}") {
            $("#profitLine1").empty().html($.getI18n('noData'));
            return false;
        }
