...
var useElecKPIPageRender = {
    ...
    initTime: function initTime() {
        ...
        // 格式化，最大和最小间隔
        var patterns = [{ name: 'yyyy-MM-dd', min: 30 * onDay, reduce: 15 * onDay, addLast: '' }, //日：默认显示15天(reduce)，最多显示30天(min)
        ...
        $('#statisticalSwitch2').unbind('click').on('click', function () {
            ...
            var addmin = void 0,
                addmax = void 0; //日历框的最小值、最大值
            var newDate = new Date(); //今天的日期
            var nNewDate = +newDate;
            if (nNewDate < +curDate + patterns[idx].reduce) {
                //超过今天，则设置最大为今天
                addmin = newDate.pattern(patterns[idx].name);
            } else {
                addmin = new Date(+curDate + patterns[idx].reduce).pattern(patterns[idx].name);
            }
            if (nNewDate < +curDate + patterns[idx].min) {
                //超过今天，则设置最大为今天
    ...
    //日月年切换
    timeSwitch: function timeSwitch() {
        var _this = this;
        var onDay = 24 * 3600 * 1000;
        // 格式化，最大和最小间隔
        var patterns = [{ name: 'yyyy-MM-dd', min: 30 * onDay, reduce: 15 * onDay, addLast: '' }, //日：默认显示15天(reduce)，最多显示30天(min)
        ...
        $('#controlBox a').unbind('click').on('click', function () {
            ...
            $this.addClass('on');
            var idx = $('#controlBox .on').attr('attr-val');
            var initNewDate = new Date(); //今天的日期
            var initAddmin = initNewDate.pattern(patterns[idx].name);
            var initCurTime = new Date(+initNewDate - patterns[idx].reduce).pattern(patterns[idx].name);
    ...
    drawKPI: function drawKPI(datas) {
