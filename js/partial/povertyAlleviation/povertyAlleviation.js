...
var povertyAlleviationRender = {
    ...
    Render: function Render() {
        ...
        $(".mainBox").children(".eachMain").each(function (index, item) {
            if (news.length % 2 == 0) {
                //news数组的长度为2的倍数的时候显示第length除以2的那一条新闻
    ...
    runLeft: function runLeft(that, width, news) {
        var activeP = $(".activeP"); //中间蓝色小圆点所在的p标签
        var preP = $(".activeP").prev("p"); //第一个小圆点所在p标签
        var nextP = $(".activeP").next("p"); //第三个小圆点所在p标签
        if (!$(".mainBox").children(".eachMain").is(":animated")) {
            if (!parseInt($(".mainBox .eachMain:last-child").css("left"))) {} else {
                that.activeIndex++;
    ...
    runRight: function runRight(that, width, news) {
        var activeP = $(".activeP"); //中间蓝色小圆点所在的p标签
        var preP = $(".activeP").prev("p"); //第一个小圆点所在p标签
        var nextP = $(".activeP").next("p"); //第三个小圆点所在p标签
        if (!$(".mainBox").children(".eachMain").is(":animated")) {
            if (!parseInt($(".mainBox .eachMain:first-child").css("left"))) {} else {
                that.activeIndex--;
                $(".mainBox").children(".eachMain").animate({ left: "+=" + width + "px" }, 1000);
                nextP.find("span").html(news[that.activeIndex + 1].time);
                activeP.find("span").html(news[that.activeIndex].time);
    ...
    //获取扶贫信息
    getPoorHouseholdsInfo: function getPoorHouseholdsInfo() {
        var that = this;
    ...
    //获取增收信息
    getIncreaseIncome: function getIncreaseIncome() {
        var that = this;
    ...
    getNewsList: function getNewsList() {
        ...
        $.http.POST('/poverty/getNewsList.do', {
            ...
            var nextP = $(".activeP").next("p"); //第三个小圆点所在p标签
            $(".mainBox").children(".eachMain").each(function (index, item) {
                if (res.body.length % 2 == 0) {
                    //res.body数组的长度为2的倍数的时候显示第length除以2的那一条新闻
    ...
    //扶贫bar
    poorNumBar: function poorNumBar(completeNum, poorTotal) {
    ...
    //电站规模scale
    scalebar: function scalebar(data) {
    ...
    //发电量统计
    generation: function generation(xData, yData, unit) {
        var option = {
            color: ['#3398DB'],
            tooltip: {
                trigger: 'axis',
            ...
            series: [{ name: '发电量',
                ...
                itemStyle: {
                    normal: {
                        barBorderRadius: 4,
                        color: new Echarts.graphic.LinearGradient(0, 0, 0, 1, [{ offset: 0, color: '#03AFFD' }, { offset: 0.5, color: '#22BEED' }, { offset: 1, color: '#33C6E4' }])
                    },
                    emphasis: {
                        barBorderRadius: 4,
    ...
    //获取电站规模
    getPlantCount: function getPlantCount() {
        var that = this;
    ...
    //新闻详情弹窗
    newsDetailTem: function newsDetailTem() {
        var tem = "<div class=\"newsDetail\"></div>";
        return tem;
    },
    //获取发电量统计
    getElectricityProfit: function getElectricityProfit() {
        var that = this;
    ...
    //获取当前用户所有电站并渲染Tabel
    getPlant: function getPlant(nameLsit) {
        var that = this;
    ...
    //专为山阳造的数据
    renderPoverty: function renderPoverty(list) {
        $(".tableContent").empty();
