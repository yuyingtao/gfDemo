...
var cameraRender = {
        ...
        $("#deleteCam").unbind().on("click", function () {
            ...
            $("#cameraTable tbody tr").each(function () {
                if ($(this).find("img").attr("src") == "images/repImages/rect1.png") {
                    var row = $("table#cameraTable tr").index($(this));
                    var cruId = _this.data[row - 1].cameraId;
    ...
    //加载树
    ztreeShow: function ztreeShow(_this, url) {
        var treeNode;
        var that = this;
    ...
    //修改摄像头信息时加载的树
    modifyAddztreeShow: function modifyAddztreeShow(_this, idName) {
        var treeNode;
    ...
    //新增摄像头信息时加载的树
    addztreeShow: function addztreeShow(_this, idName) {
        var treeNode;
        $.http.POST("/org/getOrgTree.do", { tokenId: Cookies.getCook('tokenId') }, function (result) {
            treeNode = result.body;
            // console.log(result)
    ...
    //获取表格数据
    getData: function getData(_this, obj) {
        var that = _this;
        ...
        var columns = [{
            ...
            "bSortable": false
        }, { data: "cameraName", title: $.getI18n('deviceName'), render: function render(data, type, full, meta) {
                return '<div class="td-noWrap" title="' + data + '">' + data + '</div>';
            } }, { data: "cameraSN", title: $.getI18n('sn'), render: function render(data, type, full, meta) {
                return '<div class="td-noWrap" title="' + data + '">' + data + '</div>';
            } }, { data: "manufacture", title: $.getI18n('manufacture'), render: function render(data, type, full, meta) {
                return '<div class="td-noWrap" title="' + data + '">' + data + '</div>';
            } }, { data: "cameraModel", title: $.getI18n('model'), render: function render(data, type, full, meta) {
                return '<div class="td-noWrap" title="' + data + '">' + data + '</div>';
            } }, { data: "orgName", title: $.getI18n('orgName'), render: function render(data, type, full, meta) {
                return '<div class="td-noWrap" title="' + data + '">' + data + '</div>';
            } }, { data: "plantName", title: $.getI18n('plantName'), render: function render(data, type, full, meta) {
                return '<div class="td-noWrap" title="' + data + '">' + data + '</div>';
        ...
        if (!!window.systemRole['cameraManageEdit']) {
            columns.push({ data: "cameraId", title: $.getI18n('operate'), render: function render(data) {
        ...
        _this.cameraTable = $('#cameraTable').dataTable({
            ...
            columns: columns,
            columnDefs: [{ "width": "5%", "targets": 0 }, { "width": "10%", "targets": 1 }, { "width": "10%", "targets": 2 }, { "width": "10%", "targets": 3 }, { "width": "8%", "targets": 4 }, { "width": "10%", "targets": 5 }, { "width": "10%", "targets": 6 }, { "width": "10%", "targets": 7 }, {
        ...
        $("#cameraTable tbody").on("click", ".modifyCam", function (event) {
            ...
            var row = $("table#cameraTable tr").index($(this).closest("tr"));
    ...
    //修改时获取当前组织下的电站列表
    plantIdofOrg4Camera: function plantIdofOrg4Camera(_this, select) {
    ...
    //新增时获取当前组织下的电站列表
    getPlantIdofOrg4Camera: function getPlantIdofOrg4Camera(_this, select) {
