...
var dau = {
    ...
    getData: function getData(obj) {
        ...
        var columns = [{
            data: "deviceId", "render": function render(data, type, full, meta) {
                return '<img class="checkImg" style="width: 16px;height: 16px;" data-id="' + data + '" src="images/repImages/rect.png">';
            }
        }, { data: "deviceName", title: $.getI18n('deviceName') }, { data: "modelId", title: $.getI18n('model'), width: '12%' }, { data: "sn", title: $.getI18n('sn') }, {
            data: "orgName", title: $.getI18n('orgName'), render: function render(data) {
                return '<div class="td-noWrap-dau" title="' + data + '">' + data + '</div>';
            }
        }, {
            data: "plantName", title: $.getI18n('plantName'), render: function render(data) {
                return '<div class="td-noWrap-dau" title="' + data + '">' + data + '</div>';
            }
        }, { data: "connTime", title: $.getI18n('accessTime') }, {
            data: "status", title: $.getI18n('status'), render: function render(data) {
                if (data == 1) {
        ...
        if (!!window.systemRole['dataLoggerEdit']) {
            columns.push({
                data: 'deviceId', "title": $.getI18n('operate'), render: function render(data) {
        ...
        $('#dauTable tbody').on('click', 'td a.modifyInverter', function (e) {
            ...
            var btns = [{ text: $.getI18n('cancel'), type: 'imgNoBtn', clickToClose: true, id: 'cancelOwner' }, {
                text: $.getI18n('save'), type: 'imgNoBtn', id: 'saveOwner', click: function click() {
            ...
            if (that.collType[3] == 2) {
                btns.push({
                    text: '保存并下发点表', type: 'imgNoBtn', click: function click() {
                        var obj = $('#dauForm').getForm();
                        var type = 1;
                        that.modifySaveInfo(obj, that, type, id);
                    }
                ...
                var dau = App.dialog({
                    ...
                    openEvent: function openEvent() {
                        ...
                        $.http.POST('/device/getDevicesUnderColl.do', {
                            ...
                            if (res.body) {
                                ...
                                $.each(res.body.data, function (i, val) {
                                    ...
                                    if (arr.contains(val.typeId)) {} else {
                                    ...
                                    if (arr1.contains(val.modelId)) {} else {
                                    ...
                                    if (res.body.data[i].modeDefault) {
                                        ...
                                        if (arr2.contains(val.modeId)) {} else {
                                    ...
                                    th += '<tr id="oldTr' + i + '"><td class="changeCheck"><img class="checkall" src="images/repImages/rect.png" data-id="' + val.deviceId + '" style="width: 16px;height: 16px"></td>' + '<td><input name="deviceName" value="' + val.deviceName + '" class="hasBorder"/></td>' +
                                    // '<td><input name="typeName" value="'+val.typeName+'"/><input style="display: none" name="modelId" value="'+val.modelId+'"/></td>'+
                                    '<td><select name="typeId">' + str + '</select></td>' + '<td style="width: 200px"><select name="modelId">' + str1 + '</select></td>' + '<td><input name="deviceSN" value="' + val.deviceSN + '" class="hasBorder"></td>' + '<td class="modeSelect"><select name="modeId">' + str2 + '</select></td>' + '<td class="serialPortSelect"><select name="serialPort">' + str2 + '</select></td>' + '<td><input style="display: none" name="deviceId" value="' + val.deviceId + '"><input style="display: none" name="typeId1" value="' + val.typeId + '"><input name="subDeviceOrder" value="' + val.subDeviceOrder + '" ></td>' + '</tr>';
                                });
                        ...
                        $('#addRow').unbind().on('click', function () {
                            ...
                            if (modifyIndex1 >= 5) {
                                thtml = '<tr id="trIndex' + i + '">' + '<td class="changeCheck"><img class="checkall" src="images/repImages/rect.png" style="width: 16px;height: 16px"></td>' + '<td><input name="deviceName" class="hasBorder"/></td>' + '<td><select name="typeId"><option value="">' + $.getI18n('pleaseChoose') + '</option></select></td>' + '<td style="width: 200px"><select name="modelId"><option value="">' + $.getI18n('pleaseChoose') + '</option></select></td>' + '<td><input name="deviceSN" class="hasBorder"/></td>' + '<td class="modeSelect1"><select name="modeId">' + $.getI18n('pleaseChoose') + '</select></td>' + '<td class="serialPortSelect2"><select name="serialPort">' + '<option value="1">串口1</option>' + '<option value="2" selected>串口2</option>' + '</select></td>' + '<td><input name="subDeviceOrder" i18nOnly="placeholder" i18n="' + $.getI18n('required') + '" placeholder="' + $.getI18n('required') + '" class="hasBorder" /></td>' + '</tr>';
                            } else {
                                thtml = '<tr id="trIndex' + i + '">' + '<td class="changeCheck"><img class="checkall" src="images/repImages/rect.png" style="width: 16px;height: 16px"></td>' + '<td><input name="deviceName" class="hasBorder"/></td>' + '<td><select name="typeId"><option value="">' + $.getI18n('pleaseChoose') + '</option></select></td>' + '<td style="width: 200px"><select name="modelId"><option value="">' + $.getI18n('pleaseChoose') + '</option></select></td>' + '<td><input name="deviceSN" class="hasBorder"/></td>' + '<td class="modeSelect1"><select name="modeId">' + $.getI18n('pleaseChoose') + '</select></td>' + '<td class="serialPortSelect2"><select name="serialPort">' + '<option value="1">串口1</option>' + '<option value="2">串口2</option>' + '</select></td>' + '<td><input name="subDeviceOrder" i18nOnly="placeholder" i18n="' + $.getI18n('required') + '" placeholder="' + $.getI18n('required') + '" class="hasBorder" /></td>' + '</tr>';
                            }
                        ...
                        $.http.POST('/device/getDevicesUnderColl.do', {
                            ...
                            if (res.body) {
                                ...
                                $.each(res.body.data, function (i, val) {
                                    ...
                                    th += '<tr id="oldTr' + i + '"><td class="changeCheck"><img class="checkall" src="images/repImages/rect.png" data-id="' + val.deviceId + '" style="width: 16px;height: 16px"></td>' + '<td><input disabled name="deviceName" class="hasBorder" value="' + val.deviceName + '"/></td>' +
                                    // '<td><input name="typeName" value="'+val.typeName+'"/><input style="display: none" name="modelId" value="'+val.modelId+'"/></td>'+
                                    '<td><select name="typeId" disabled>' + str + '</select></td>' + '<td><select name="modelId" disabled>' + str1 + '</select></td>' + '<td><input disabled name="deviceSN" class="hasBorder" value="' + val.deviceSN + '"></td>' + '<td class="modeSelect"><select name="modeId" disabled>' + str2 + '</select></td>' + '<td class="serialPortSelect"><select name="serialPort" disabled>' + str2 + '</select></td>' + '<td><input style="display: none" name="deviceId" value="' + val.deviceId + '"><input name="subDeviceOrder" value="' + val.subDeviceOrder + '" readonly></td>' + '</tr>';
                                });
                                $('.addTable tbody').append(th);
                                if (that.collType[3] == 2) {
    ...
    dauTep: function dauTep() {
        var dauTep = '<form id="dauForm">' + '<div class="dauDiv" style="margin-top: 0px"><b>*</b><lable>' + $.getI18n('dauName') + '<span>：</span></label><input name="deviceName" id="" required></div>' + '<div class="dauDiv" style="margin-top: 0px"><b>*</b><lable>' + $.getI18n('dauSn') + '<span>：</span></label><input i18nOnly="placeholder" i18n="' + $.getI18n('inputDauSn') + '" placeholder="' + $.getI18n('inputDauSn') + '" name="sn" required/></div>' + '<div class="dauDiv" style="margin-top: 0px"><b>*</b><lable>' + $.getI18n('dauModel') + '<span style="visibility: hidden">看</span><span>：</span></label><select name="topModelId" required></select></div>' + '<div class="dauDiv"><b>*</b><lable>' + $.getI18n('orgName') + '<span>：</span></label><input i18nOnly="placeholder" i18n="' + $.getI18n('chooseOrgName') + '" placeholder="' + $.getI18n('chooseOrgName') + '" name="orgName" required style="cursor: pointer"/></div>' + '<div class="dauDiv"><b style="color: #fff">*</b><lable>' + $.getI18n('plantName') + '<span>：</span></label><select name="plantId" id=""></select></div>' + '<div class="dauDiv maxLengthInput"><b>*</b><lable>接入设备数<span>：</span></label><input name="maxLength" class="checkNumber" placeholder="请输入整数" id="" required/></div>' +
        // '<div class="dauDiv"><b>*</b><lable>'+$.getI18n('ass.maxNumber')+'<span>：</span></label><input type="text" class="changeRow"></div>'+
        '</form>' + '<div>' + '<div class="ownerHead">' + '<span>' + $.getI18n('subDevInfo') + '</span>' + '<span id="addRow">' + $.getI18n('add') + '</span>' + '<span id="delRow">' + $.getI18n('delete') + '</span>' + '</div>' + '<table style="margin-top: 10px" class="addTable device">' + '<thead>' + '<tr>' + '<th class="checkAll">' + '<img class="checkall" src="images/repImages/rect.png" style="width: 16px;height: 16px">' + '</th>' + '<th>' + $.getI18n('subDev') + '</th>' + '<th>' + $.getI18n('deviceType') + '</th>' + '<th>' + $.getI18n('model') + '</th>' + '<th>' + $.getI18n('devSn') + '</th>' + '<th class="usePoint">' + $.getI18n('usePoint') + '</th>' + '<th class="serialPort">串口号</th>' + '<th class="devOrder">' + $.getI18n('devOrder') + '</th>' + '</tr>' + '</thead>' + '<tbody></tbody>' + '</table>';
        '</div>';
        return dauTep;
    },
    //修改数采弹窗 非sysadmin
    dauTep1: function dauTep1() {
        var dauTep = '<form id="dauForm1">' + '<div class="dauDiv"><b style="color: #fff">*</b><lable>' + $.getI18n('dauName') + '<span>：</span></label><input name="deviceName" id=""></div>' + '<div class="dauDiv"><b></b><lable>' + $.getI18n('dauSn') + '<span>：</span></label><input i18nOnly="placeholder" i18n="' + $.getI18n('inputDauSn') + '" placeholder="' + $.getI18n('inputDauSn') + '" name="sn" disabled/></div>' + '<div class="dauDiv" ><b></b><lable>' + $.getI18n('dauModel') + '<span>：</span></label><select name="topModelId" disabled></select></div>' + '<div class="dauDiv"><b></b><lable>' + $.getI18n('orgName') + '<span>：</span></label><input i18nOnly="placeholder" i18n="' + $.getI18n('chooseOrgName') + '" placeholder="' + $.getI18n('chooseOrgName') + '" name="orgName" disabled style="cursor: pointer"/></div>' + '<div class="dauDiv"><b></b><lable>' + $.getI18n('plantName') + '<span>：</span></label><select name="plantId" id="" disabled></select></div>' + '<div class="dauDiv maxLengthInput"><lable>接入设备数<span>：</span></label><input name="maxLength" class="checkNumber" placeholder="请输入整数" id="" disabled/></div>' + '</form>' + '<div>' + '<div class="ownerHead">' + '<span>' + $.getI18n('subDevInfo') + '</span>' +
        // '<span id="addRow">新增</span>'+
        // '<span id="delRow">删除</span>'+
        '</div>' + '<table style="margin-top: 10px" class="addTable device disabledSelect">' + '<thead>' + '<tr>' + '<th class="checkAll">' + '<img class="checkall" src="images/repImages/rect.png" style="width: 16px;height: 16px">' + '</th>' + '<th>' + $.getI18n('subDev') + '</th>' + '<th>' + $.getI18n('deviceType') + '</th>' + '<th>' + $.getI18n('model') + '</th>' + '<th>' + $.getI18n('devSn') + '</th>' + '<th>' + $.getI18n('usePoint') + '</th>' + '<th>' + $.getI18n('devOrder') + '</th>' + '</tr>' + '</thead>' + '<tbody></tbody>' + '</table>';
        '</div>';
        return dauTep;
    },
    //新增数采
    addNewDau: function addNewDau() {
        var that = this;
        var dau = App.dialog({
                        ...
                        if (sn.val().length >= 4) {
                            if (sn.val()[3] == 1) {
                                ...
                                for (var n = 0; n < rowNum; n++) {
                                    trHtml += '<tr id="trIndexRow' + n + '">' + '<td class="changeCheck"><img class="" src="images/repImages/rect.png" style="width: 16px;height: 16px"></td>' + '<td><input name="subDeviceName" class="hasBorder"/></td>' + '<td class="getTypeName"><select name="typeId"></select></td>' + '<td style="width: 200px"><select name="modelId"></select></td>' + '<td><input name="subDeviceSN" class="hasBorder"/></td>' + '<td ><select name="modeId"></select></td>' + '<td><input name="subDeviceOrder" i18nOyly="placeholder" i18n="' + $.getI18n('required') + '" placeholder="' + $.getI18n('required') + '" class="hasBorder" value="' + (n + 1) + '"/></td>' + '</tr>';
                                }
                                ...
                                for (var n = 0; n < rowNum; n++) {
                                    if (n <= 4) {
                                        trHtml += '<tr id="trIndexRow' + n + '">' + '<td class="changeCheck"><img class="" src="images/repImages/rect.png" style="width: 16px;height: 16px"></td>' + '<td><input name="subDeviceName" class="hasBorder"/></td>' + '<td class="getTypeName"><select name="typeId"></select></td>' + '<td style="width: 200px"><select name="modelId"></select></td>' + '<td><input name="subDeviceSN" class="hasBorder"/></td>' + '<td><select name="serialPort">' + '<option value="1" selected>串口1</option>' + '<option value="2">串口2</option>' + '</select></td>' +
                                        // '<td><input name="serialPort" class="hasBorder"/></td>'+
                                        '<td><input name="subDeviceOrder" i18nOyly="placeholder" i18n="' + $.getI18n('required') + '" placeholder="' + $.getI18n('required') + '" class="hasBorder" value="' + (n + 1) + '"/></td>' + '</tr>';
                                    } else {
                                        trHtml += '<tr id="trIndexRow' + n + '">' + '<td class="changeCheck"><img class="" src="images/repImages/rect.png" style="width: 16px;height: 16px"></td>' + '<td><input name="subDeviceName" class="hasBorder"/></td>' + '<td class="getTypeName"><select name="typeId"></select></td>' + '<td style="width: 200px"><select name="modelId"></select></td>' + '<td><input name="subDeviceSN" class="hasBorder"/></td>' + '<td><select name="serialPort">' + '<option value="1">串口1</option>' + '<option value="2" selected>串口2</option>' + '</select></td>' +
                                        // '<td><input name="serialPort" class="hasBorder"/></td>'+
                                        '<td><input name="subDeviceOrder" i18nOyly="placeholder" i18n="' + $.getI18n('required') + '" placeholder="' + $.getI18n('required') + '" class="hasBorder" value="' + (n - 4) + '"/></td>' + '</tr>';
                                    }
                                }
                                $('.addTable tbody').append(trHtml);
                ...
                $('#addRow').unbind().on('click', function () {
                    ...
                    if (sn.val().length >= 4) {
                        if (sn.val()[3] == 1) {
                            ...
                            thtml = '<tr id="trIndex' + i + '">' + '<td class="changeCheck"><img class="" src="images/repImages/rect.png" style="width: 16px;height: 16px"></td>' + '<td><input name="subDeviceName" class="hasBorder"/></td>' + '<td><select name="typeId"></select></td>' + '<td style="width: 200px"><select name="modelId"></select></td>' + '<td><input name="subDeviceSN" class="hasBorder"/></td>' + '<td><select name="modeId"></select></td>' + '<td><input name="subDeviceOrder" i18nOyly="placeholder" i18n="' + $.getI18n('required') + '" placeholder="' + $.getI18n('required') + '" class="hasBorder checkNumber"value="' + (trLength + 1) + '"/></td>' + '</tr>';
                            $('.addTable tbody').append(thtml);
                            ...
                            if (index1 >= 5) {
                                thtml = '<tr id="trIndex' + i + '">' + '<td class="changeCheck"><img class="" src="images/repImages/rect.png" style="width: 16px;height: 16px"></td>' + '<td><input name="subDeviceName" class="hasBorder"/></td>' + '<td><select name="typeId"></select></td>' + '<td style="width: 200px"><select name="modelId"></select></td>' + '<td><input name="subDeviceSN" class="hasBorder"/></td>' + '<td><select name="serialPort">' + '<option value="1">串口1</option>' + '<option value="2" selected>串口2</option>' + '</select></td>' + '<td><input name="subDeviceOrder" i18nOyly="placeholder" i18n="' + $.getI18n('required') + '" placeholder="' + $.getI18n('required') + '" class="hasBorder checkNumber"value="' + (index2 + 1) + '"/></td>' + '</tr>';
                            } else if (index2 >= 5) {
                                thtml = '<tr id="trIndex' + i + '">' + '<td class="changeCheck"><img class="" src="images/repImages/rect.png" style="width: 16px;height: 16px"></td>' + '<td><input name="subDeviceName" class="hasBorder"/></td>' + '<td><select name="typeId"></select></td>' + '<td style="width: 200px"><select name="modelId"></select></td>' + '<td><input name="subDeviceSN" class="hasBorder"/></td>' + '<td><select name="serialPort">' + '<option value="1" selected>串口1</option>' + '<option value="2">串口2</option>' + '</select></td>' + '<td><input name="subDeviceOrder" i18nOyly="placeholder" i18n="' + $.getI18n('required') + '" placeholder="' + $.getI18n('required') + '" class="hasBorder checkNumber"value="' + (index1 + 1) + '"/></td>' + '</tr>';
                            } else {
                                thtml = '<tr id="trIndex' + i + '">' + '<td class="changeCheck"><img class="" src="images/repImages/rect.png" style="width: 16px;height: 16px"></td>' + '<td><input name="subDeviceName" class="hasBorder"/></td>' + '<td><select name="typeId"></select></td>' + '<td style="width: 200px"><select name="modelId"></select></td>' + '<td><input name="subDeviceSN" class="hasBorder"/></td>' + '<td><select name="serialPort">' + '<option value="1" selected>串口1</option>' + '<option value="2">串口2</option>' + '</select></td>' + '<td><input name="subDeviceOrder" i18nOyly="placeholder" i18n="' + $.getI18n('required') + '" placeholder="' + $.getI18n('required') + '" class="hasBorder checkNumber"value="' + (index1 + 1) + '"/></td>' + '</tr>';
                            }
                            $('.addTable tbody').append(thtml);
    ...
    //数采详情弹窗
    detailDauTep: function detailDauTep() {
        var dauDetailTep = '<form id="dauDetailForm">' + '<div class="dauDiv"><lable>' + $.getI18n('dauModel') + '<span>：</span></label><input  name="modelId" disabled/></div>' + '<div class="dauDiv"><lable>' + $.getI18n('dauSn') + '<span>：</span></label><input  name="sn" disabled/></div>' + '<div class="dauDiv" ><lable>' + $.getI18n('dauName') + '<span>：</span></label><input  name="deviceName" disabled/></div>' + '<div class="dauDiv"><lable>' + $.getI18n('orgName') + '<span>：</span></label><input name="orgName" disabled/></div>' + '<div class="dauDiv"><lable>' + $.getI18n('plantName') + '<span>：</span></label><input type="text" disabled name="plantName"></div>' + '<div class="dauDiv maxLengthInput"><lable>接入设备数<span>：</span></label><input name="maxLength" class="checkNumber" id="" disabled/></div>' + '</form>' + '<div>' + '<div class="ownerHead">' + '<span>' + $.getI18n('subDevInfo') + '</span>' + '</div>' + '<table style="margin-top: 10px" class="addTable device" id="dauDetailTable">' + '<thead>' + '<tr>' + '<th>' + $.getI18n('subDev') + '</th>' + '<th>' + $.getI18n('deviceType') + '</th>' + '<th>' + $.getI18n('model') + '</th>' + '<th>' + $.getI18n('devSn') + '</th>' + '<th class="detailSer">' + $.getI18n('usePoint') + '</th>' + '<th class="detailOrder">' + $.getI18n('devOrder') + '</th>' + '</tr>' + '</thead>' + '<tbody></tbody>' + '</table>';
        '</div>';
        return dauDetailTep;
    },
    //详情弹窗表格数据
    dauDetailTableData: function dauDetailTableData(obj, snType) {
        if (this.dauDetailTable) {
            this.dauDetailTable.fnClearTable(false);
            this.dauDetailTable.fnDestroy();
        }
        var columns = [{ data: "deviceName", title: $.getI18n('subDev') }, { data: "typeName", title: $.getI18n('deviceType') }, { data: "modelName", title: $.getI18n('model') }, { data: "deviceSN", title: $.getI18n('devSn') }, { data: "modeName", title: $.getI18n('usePoint') }, { data: "subDeviceOrder", title: $.getI18n('devOrder') }];
        if (snType == 2) {
            columns = [{ data: "deviceName", title: $.getI18n('subDev') }, { data: "typeName", title: $.getI18n('deviceType') }, { data: "modelName", title: $.getI18n('model') }, { data: "deviceSN", title: $.getI18n('devSn') }, { data: "serialPortId", title: $.getI18n('usePoint') }, { data: "subDeviceOrder", title: $.getI18n('devOrder') }];
        }
    ...
    //加载树
    ztreeShow: function ztreeShow(url) {
        var treeNode;
        var that = this;
    ...
    //级联选择1  获取设备类型
    getSubDeviceType: function getSubDeviceType(that) {
    ...
    //级联选择2  获取设备型号
    getSubModel: function getSubModel(that, typeId) {
        $.http.POST('/device/getDeviceChoiceTree.do', {
            requestType: 1,
            modelId: '',
            typeId: typeId,
    ...
    //级联选择3  获取电表
    getSubPointTable: function getSubPointTable(that, typeId, modelId) {
        $.http.POST('/device/getDeviceChoiceTree.do', {
            requestType: 2,
            modelId: modelId,
            typeId: typeId,
    ...
    getDauInfo: function getDauInfo(type, that) {
        $.http.POST('/device/getDeviceChoiceTree.do', {
            requestType: 0,
            tokenId: Cookies.getCook('tokenId'),
            modelId: '',
    ...
    //所属组织弹窗信息
    orgTep: function orgTep() {
        var orgTep = '<div id="orgTree" class="ztree"></div>';
        return orgTep;
    },
    //删除下联设备
    deleteSubDevice: function deleteSubDevice() {
        var that = this;
    ...
    //切换选中状态
    changeRect: function changeRect(_this) {
    ...
    //查询出所有数采的名称
    getAllDauName: function getAllDauName() {
        //查询出所有数采名称，并赋值
        var that = this;
        $.http.POST('/device/getAllCollNames.do', { tokenId: Cookies.getCook('tokenId') }, function (res) {
            var op = '';
    ...
    //获取表格tr的长度
    getTrLen: function getTrLen(className) {
    ...
    //新增数采时保存当前信息
    saveInfo: function saveInfo(obj, that, type) {
        //获取每一行的值
        var subDevice = [];
        $('.addTable tbody tr').each(function (i) {
            var object = 'obj' + (i + 1);
    ...
    //管理员登录修改数采时保存信息
    modifySaveInfo: function modifySaveInfo(obj, that, type, id) {
        var trLength = $('.addTable tbody tr ').length;
