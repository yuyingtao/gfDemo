...
var plantInfo = {
    ...
    Render: function Render() {
        ...
        $('#lookPrice').unbind().on('click', function () {
            ...
            var priceInfos = App.dialog({
                ...
                openEvent: function openEvent() {
                    $.http.POST('/plantInfo/getPlantPriceDetail.do', { id: that.plantObj.priceId, tokenId: Cookies.getCook('tokenId') }, function (res) {
                        ...
                        $.each(res.body, function (ind, value) {
                            th += '<tr>' + '<td style="text-indent: 30px">' + Number(ind + 1) + '</td>' + '<td>' + value.time + '</td>' + '<td>' + value.pos + '</td>' + '<td>' + value.neg + '</td>';
                        });
                ...
                closeEvent: null,
                isdrag: true,
                buttons: [{ text: $.getI18n('close'), type: 'imgNoBtn', clickToClose: true, id: 'cancelOwner', click: function click() {} }]
            });
        });
        //组织、地区分组切换
    ...
    //获取电站基本信息和业主信息
    getPlantInfo: function getPlantInfo(obj) {
        var that = this;
    ...
    //绘制业主表格
    drawTable: function drawTable(datas) {
        var html = '';
        var that = this;
        $('#ownerTable tbody').empty();
        if (datas.length > 0) {
            $.each(datas, function (i, val) {
                html += '<tr>' + '<td style="text-indent: 30px">' + Number(i + 1) + '</td>' + '<td>' + val.loginId + '</td>' + '<td>' + val.userName + '</td>' + '<td>' + val.userTel + '</td>';
            ...
            $('#ownerTable tbody tr td a.modifyPwd').unbind().on('click', function () {
                ...
                // '</div>'+
                '<div class="form-group1">' + '<b>*' + '</b>' + '<label>' + $.getI18n('newPwd') + '' + '</label>' + '<input name="NewPassword" type="password" id="NewPassword"/>' + '<span class="NewPassword">你好</span>' + '</div>' + '<div class="form-group1">' + '<b>*' + '</b>' + '<label>' + $.getI18n('confirmPwd') + '' + '</label>' + '<input name="repNewPassword" type="password" id="repNewPassword"/>' + '<span class="repNewPassword">你好</span>' + '</div>' + '</form>';
                pd = App.dialog({
    ...
    //绘制电站信息数据
    drawPlant: function drawPlant(datas) {
    ...
    //新增业主信息弹窗模板
    newOwnerTep: function newOwnerTep() {
        //弹窗模板
        this.ownerContent = '<form id="ownerInformation" class="plantForm">' + '<div class="leftInfo fl">' + '<div class="item-group-plant">' + '<b>*' + '</b>' + '<label>' + $.getI18n('orgName') + '<span>：</span>' + '</label>' + '<input name="org" required id="org" readonly/>' + '</div>' + '<div class="item-group-plant">' + '<b>*' + '</b>' + '<label>' + $.getI18n('username') + '<span>：</span>' + '</label>' + '<input name="userName" required/>' + '</div>' + '<div class="item-group-plant">' + '<b>*' + '</b>' + '<label>' + $.getI18n('role') + '<span>：</span>' + '</label>' + '<input name="role" required readonly value="' + $.getI18n('plantOwner') + '">' + '</div>' + '<div class="item-group-plant">' + '<b>*' + '</b>' + '<label>' + $.getI18n('plantName') + '<span>：</span>' + '</label>' + '<input name="plantId" required readonly/>' + '</div>' + '<div class="item-group-plant">' + '<b>*' + '</b>' + '<label>' + $.getI18n('telephone') + '<span>：</span>' + '</label>' + '<input name="userTel" class="mobile" required readonly/>' + '</div>' + '<div class="item-group-plant">' + '<b>' + '</b>' + '<label>' + $.getI18n('email') + '<span>：</span>' + '</label>' + '<input name="email" type="email" class="checkAddRepeatEmail"/>' + '</div>' + '<div class="item-group-plant">' + '<b>' + '</b>' + '<label style="position: absolute;left: 50px;line-height: 34px">' + $.getI18n('sysId') + '<span>：</span>' + '</label>' + '<input style="position:absolute;left: 120px;line-height: 34px" name="loginId" i18nOnly="placeholder" i18n="' + $.getI18n("inputTel") + '" placeholder="' + $.getI18n("inputTel") + '" class="sysTel checkAddRepeatTel"/>' + '<span class="sysSpan" style="position:absolute;right: 36px;line-height: 34px;top:0;"><label></label>' + $.getI18n('useTel') + '</span>' + '</div>' + '</div>' + '<div class="rightInfo fr">' + '<div class="item-group-plant">' + '<b>*' + '</b>' + '<label>' + $.getI18n('accountPwd') + '<span>：</span>' + '</label>' + '<input name="password" type="password" class="firPwd" class="passwordCheck" required/>' + '</div>' + '<div class="item-group-plant">' + '<b>*' + '</b>' + '<label>' + $.getI18n('confirmPwd') + '<span>：</span>' + '</label>' + '<input type="password" class="secPwd passwordCheck" required/>' + '</div>' + '<div class="item-group-plant orgStatus">' + '<b>' + '</b>' + '<label>' + $.getI18n('accountStatus') + '<span>：</span>' + '</label>' + '<i><image src="/images/setting/plantManage/enabled.png" class="statusImg"></image></i>' + '</div>' + '<div class="item-group-plant userPic">' + '<b>' + '</b>' + '<label style="position: absolute; top: 0;left: 23px">' + $.getI18n('userImg') + '<span>：</span>' + '</label>' + '<i style="position: absolute;left: 100px;display: inline-block">' + '<div class="uploadImgBox"><image id="addUploadOwnerImg" src="/images/setting/upload.png"></image>\n' + '<img id="closeImageOwner" src="/images/setting/close.png" style="display:none;position: absolute;right: 0;z-index: 100;top: 0;width: 20px;height: 20px;"/>' + '<input id = "addOwnerImg" type="file" accept="image/jpeg,image/gif,image/png" />' + '</div>' + '<span style="position:absolute;bottom:0">' + $.getI18n('suggestOne') + '</span></i>' + '</div>' + '</div>' + '</form>';
        return this.ownerContent;
    },
    //输入业主电话弹窗
    ownerPhoneNumTep: function ownerPhoneNumTep() {
    ...
    //修改业主弹窗
    modifyTep: function modifyTep() {
        this.modifyOwnerContent = '<form id="modifyOwner" class="plantForm">' + '<div class="leftInfo fl">' + '<div class="item-group-plant">' + '<b>*' + '</b>' + '<label>' + $.getI18n('orgName') + '<span>：</span>' + '</label>' + '<input name="org" required id="org" readonly/>' + '</div>' + '<div class="item-group-plant">' + '<b>*' + '</b>' + '<label>' + $.getI18n('username') + '<span>：</span>' + '</label>' + '<input name="userName" required/>' + '</div>' + '<div class="item-group-plant">' + '<b>*' + '</b>' + '<label>' + $.getI18n('role') + '<span>：</span>' + '</label>' + '<input name="role" required readonly>' + '</div>' + '<div class="item-group-plant">' + '<b>*' + '</b>' + '<label>' + $.getI18n('plantName') + '<span>：</span>' + '</label>' + '<input name="plantId" required readonly/>' + '</div>' + '<div class="item-group-plant">' + '<b>*' + '</b>' + '<label>' + $.getI18n('telephone') + '<span>：</span>' + '</label>' + '<input name="userTel" class="mobile checkRepeatTel" required/>' + '</div>' + '</div>' + '<div class="rightInfo fr">' + '<div class="item-group-plant">' + '<b>' + '</b>' + '<label>' + $.getI18n('email') + '<span>：</span>' + '</label>' + '<input name="email" type="email" class="checkRepeatEmail"/>' + '</div>' + '<div class="item-group-plant" style="margin-bottom: 18px">' + '<b>' + '</b>' + '<label style="position: absolute;left: 28px;line-height: 34px">' + $.getI18n('sysId') + '<span>：</span>>' + '</label>' + '<input name="loginId" style="position:absolute;left: 100px;line-height: 34px" i18nOnly="placeholder" i18n="' + $.getI18n("inputTel") + '"  placeholder="' + $.getI18n("inputTel") + '" class="sysTel checkRepeatTel"/>' + '<span class="sysSpan" style="position:absolute;right: 22px;line-height: 34px;top:0;"><label class="notReact"></label>' + $.getI18n('useTel') + '</span>' + '</div>' + '<div class="item-group-plant orgStatus">' + '<b>' + '</b>' + '<label>' + $.getI18n('accountStatus') + '<span>：</span>' + '</label>' + '<i><image src="/images/setting/plantManage/enabled.png" class="statusImg"></image></i>' + '</div>' + '<div class="item-group-plant userPic">' + '<b>' + '</b>' + '<label style="position: absolute; top: 0;left: 23px">' + $.getI18n('userImg') + '<p style="display: inline-block">：</p>' + '</label>' + '<i style="position: absolute;left: 100px;display: inline-block">' + '<div class="uploadImgBox"><image id="modifyUploadOwnerImg" src="/images/setting/upload.png"></image>\n' + '<img id="closeImageOwner1" src="/images/setting/close.png" style="display:none;position: absolute;right: 0;z-index: 100;top: 0;width: 20px;height: 20px;"/>' + '<input id = "modifyUpOwnerImg" type="file" accept="image/jpeg,image/gif,image/png" />' + '</div>' + '<span style="position:absolute;bottom:0">' + $.getI18n('suggestOne') + '</span></i>' + '</div>' + '</div>' + '</form>';
        return this.modifyOwnerContent;
    },
    //新增业主
    addNewOwner: function addNewOwner() {
        var that = this;
    ...
    //修改电站信息
    modifyPlant: function modifyPlant() {
        var plantInfo = '';
        var that = this;
        //var administrative =[];
        ...
        var plantContent = '<form id="plantInfoOfPlant" class="plantForm">' + '<div class="leftInfo fl">' + '<div class="item-group-plant">' + '<b>*' + '</b>' + '<label>' + $.getI18n('plantName') + '<span>：</span>' + '</label>' + '<input name="plantName" class="checkPlantName" required/>' + '</div>' + '<div class="item-group-plant">' + '<b>*' + '</b>' + '<label>' + $.getI18n('capacity') + '<span>：</span>' + '</label>' + '<input class="capacity checkNumber" name="capacity" required>' + '<select class="capSelect" name="capUnit">' +
        // '<option value="MW">MW</option>'+
        // '<option value="KW">KW</option>'+
        // '<option value="GW">GW</option>'+
        '</select>' + '</div>' + '<div class="item-group-plant">' + '<b>*' + '</b>' + '<label>' + $.getI18n('plantAddress') + '<span>：</span>' + '</label>' + '<input class="addrInput" name="plantAddr" required readonly>' + '<a id="getLocation">' + $.getI18n('mapPosition') + '</a>' + '</div>' + '<div class="item-group-plant">' + '<b>' + '</b>' + '<label>' + $.getI18n('detailedAddress') + '<span>：</span>' + '</label>' + '<input class="" name="plantFullAddress">' + '</div>' + '<div class="item-group-plant">' + '<b>*' + '</b>' + '<label>' + $.getI18n('location') + '<span>：</span>' + '</label>' + '<input name="loaction" required readonly/>' + '</div>' + '<div class="item-group-plant">' + '<b>*' + '</b>' + '<label>' + $.getI18n('areaCovered') + '<span>：</span>' + '</label>' + '<input name="plantArea" class="plantArea checkNumber" required/>㎡' + '</div>' +
        ...
        // '</div>'+
        '<div class="item-group-plant">' + '<b>' + '</b>' + '<label class="ptInfoLabel" style="position: absolute; top: 0;left: 47px">' + $.getI18n('introduction') + '<span>：</span>' + '</label>' + '<textarea style="position: absolute;left: 125px;resize:none" i18nOnly="placeholder" i18n="' + $.getI18n("inputPlantIntroduction") + '" placeholder="' + $.getI18n("inputPlantIntroduction") + '" name="plantIntroduction"></textarea>' + '</div>' + '</div>' + '<div class="rightInfo fr">' + '<div class="item-group-plant">' + '<b>*' + '</b>' + '<label>' + $.getI18n('accessTime') + '<span>：</span>' + '</label>' + '<input style="cursor:pointer;border:1px solid #ebebeb" class="time" name="gridConnectedDate" onclick="WdatePicker({readOnly:true,dateFmt:\'yyyy-MM-dd\'})" required/>' + '<i class="timeLine"></i>' + '</div>' + '<div class="item-group-plant">' + '<b>*' + '</b>' + '<label>' + $.getI18n('plantType') + '<span>：</span>' + '</label>' + '<select class="select1" style="margin-ringht:10px;" name="plantTypeA" required>' + '</select>' + '<select class="select1" name="plantTypeB" required>' + '</select>' + '</div>' + '<div class="item-group-plant">' + '<b>*' + '</b>' + '<label>' + $.getI18n('price') + '<span style="visibility: hidden">你好</span><span>：</span>' + '</label>' +
        ...
        '<select name="plantPrice" class="priceSee"></select><a class="priceSee" id="priceSee">查看电价规则</a>' + '</div>' + '<div class="item-group-plant userPic">' + '<b>' + '</b>' + '<label>' + $.getI18n('plantImg') + '<p style="display:inline-block">：</p>' + '</label>' + '<input style="border:none;color:#FF0E26" value="' + $.getI18n('suggestImg') + '">' +
        ...
        '</div>' + '<div class="item-group-plant plantImg">' + '<i style="display: inline-block;width:33%;float:left;">' + '<div class="uploadImgBox"><image id="modifyUploadImgBox1" src="/images/setting/upload.png"></image>\n' + '<img class="closeImage num1" src="/images/setting/close.png" style="display:none;position: absolute;right: 0;z-index: 100;top: 0;width: 20px;height: 20px;"/>' + '<input id = "modifyUp_img1" type="file" accept="image/jpeg,image/gif,image/png" />' + '</div></i>' + '<i style="display: inline-block;width:33%;float:left;">' + '<div class="uploadImgBox"><image id="modifyUploadImgBox2" src="/images/setting/upload.png"></image>\n' + '<img class="closeImage num2" src="/images/setting/close.png" style="display:none;position: absolute;right: 0;z-index: 100;top: 0;width: 20px;height: 20px;"/>' + '<input id = "modifyUp_img2" type="file" accept="image/jpeg,image/gif,image/png" />' + '</div></i>' + '<i style="display: inline-block;width:33%;float:left;">' + '<div class="uploadImgBox"><image id="modifyUploadImgBox3" src="/images/setting/upload.png"></image>\n' + '<img class="closeImage num3" src="/images/setting/close.png" style="display:none;position: absolute;right: 0;z-index: 100;top: 0;width: 20px;height: 20px;"/>' + '<input id = "modifyUp_img3" type="file" accept="image/jpeg,image/gif,image/png" />' + '</div></i>' + '</div>' + '</div>' + '</form>';
            ...
            plantInfo = App.dialog({
                ...
                openEvent: function openEvent() {
                    ...
                    $('#priceSee').unbind().on('click', function () {
                        var priceVal = $('select[name="plantPrice"]').val();
                        var priceInfos = App.dialog({
                            title: '电价详情',
                            width: 500,
                            height: 500,
                    ...
                    $('#getLocation').unbind().on('click', function () {
                        ...
                        var location = App.dialog({
                            ...
                            openEvent: function openEvent() {
                                ...
                                qq.maps.event.addListener(qMap, 'click', function (e) {
            ...
            $.http.POST('/plantInfo/getPlantInfo.do', { plantId: that.plantId, tokenId: Cookies.getCook('tokenId') }, function (res) {
                ...
                switch (length) {
                    case 1:
                        $('.num1').show();break;
                    case 2:
                        $('.num1').show();$('.num2').show();break;
                    case 3:
                        $('.closeImage').show();break;
                }
    ...
    //组织分组
    ztreeShow: function ztreeShow(url, chooseId) {
        var treeNode;
        var that = this;
    ...
    //地理位置弹窗模板--修改电站信息
    locationMap: function locationMap() {
        var mapL = '<div id="mapLocation">' + '<div id="map" style="width:1000px;height: 600px;position: absolute;left: 0;top: 0;"></div>' + '<div id="locInfo" style="z-index: 9999; width: 400px;position: absolute;left: 10px;top:10px;">' + '<div style="border: 4px solid rgb(149,218,243);border-radius: 4px;height: 96px">' + '<p><i><img src="/images/setting/plantManage/tellurion.png" alt=""></i>' + $.getI18n('location') + '<span>&nbsp;：</span><input/></p>' + '<p><i><img src="/images/setting/plantManage/site.png" alt=""></i><span>地<span style="visibility: hidden">你好</span>址</span>：<input/></p>' + '</div>' + '<div class="jg"></div>' + '<p style="background: #fff;color: #666;border: 4px solid rgb(220,218,206);border-radius: 4px"><span style="margin-left: 10px">' + $.getI18n('detailedAddress') + '</span><span>：</span><input style="background: #fff"/></p>' + '</div>' + '<div style="z-index: 9999; width: 344px;position: absolute;right: 130px;top:10px;background: #fff"><input id="modifyAddress" style="display: inline-block;width: 300px;height: 34px;outline: none;border: none" type="text" i18nOnly="placeholder" i18n="' + $.getI18n("cityAddress") + '" placeholder="' + $.getI18n("cityAddress") + '"> <img id="modifySearchMap" src="/images/newMain/search.png" alt=""></div>' + '</div>';
        return mapL;
    },
    //静态地址
    locationMap2: function locationMap2() {
        var mapL = '<div id="mapLocation">' + '<div id="map" style="width:1000px;height: 600px;position: absolute;left: 0;top: 0;"></div>' + '<div id="locInfo" style="z-index: 9999; width: 400px;position: absolute;left: 10px;top:10px;">' + '<div style="border: 4px solid rgb(149,218,243);border-radius: 4px;height: 96px">' + '<p><i><img src="/images/setting/plantManage/tellurion.png" alt=""></i>' + $.getI18n('location') + '<span>&nbsp;：</span><input/></p>' + '<p><i><img src="/images/setting/plantManage/site.png" alt=""></i><span>地<span style="visibility: hidden">你好</span>址</span>：<input/></p>' + '</div>' + '<div class="jg"></div>' + '<p style="background: #fff;color: #666;border: 4px solid rgb(220,218,206);border-radius: 4px"><span style="margin-left: 10px">' + $.getI18n('detailedAddress') + '</span><span>：</span><input style="background: #fff"/></p>' + '</div>' + '</div>';
        return mapL;
    },
    //地理位置弹窗模板--新建电站信息
    locationMap1: function locationMap1() {
        var mapL = '<div id="mapLocation1">' + '<div id="map1" style="width:1000px;height: 600px;position: absolute;left: 0;top: 0;"></div>' + '<div id="locInfo1" style="z-index: 9999; width: 400px;position: absolute;left: 10px;top:10px;">' + '<div style="border: 4px solid rgb(149,218,243);border-radius: 4px;height: 96px">' + '<p><i><img src="/images/setting/plantManage/tellurion.png" alt=""></i>' + $.getI18n('location') + '<span>&nbsp;：</span><input readonly/></p>' + '<p><i><img src="/images/setting/plantManage/site.png" alt=""></i><span>地<span style="visibility: hidden">你好</span>址</span>：<input readonly/></p>' + '</div>' + '<div class="jg"></div>' + '<p style="background: #fff;color: #666;border: 4px solid rgb(220,218,206);border-radius: 4px"><span style="margin-left: 10px">' + $.getI18n('detailedAddress') + '</span><span>：</span><input style="background: #fff"/></p>' + '</div>' + '<div style="z-index: 9999; width: 344px;position: absolute;right: 130px;top:10px;background: #fff"><input id="addAddress" style="display: inline-block;width: 300px;height: 34px;outline: none;border: none" type="text" i18nOnly="placeholder" i18n="' + $.getI18n("cityAddress") + '" placeholder="' + $.getI18n("cityAddress") + '"> <img id="AddSearchMap" src="/images/newMain/search.png" alt=""></div>' + '</div>';
        return mapL;
    },
    //定位查看静态信息
    staticLocation: function staticLocation(data) {
        var map;
        var location = App.dialog({
            id: 'locationMap',
    ...
    //预览图片
    fileUpLoad: function fileUpLoad(_this, upBtn, imgShowBox) {
        var that = this;
        ...
        if (!/image\/\w+/.test(file.type)) {
            //html中已经用accept='image/*'限制上传的是图片了，此处判断可省略
            App.alert("文件必须为图片！");
            return false;
        }
        if (!FileReader) {
    ...
    //图片上传多张 html在页面上
    uploadImg: function uploadImg(ids, _this) {
        var that = this;
        var pic = [];
    ...
    //图片上传多张 html是弹窗
    uploadImg1: function uploadImg1(ids, _this, imgUrl) {
        var that = this;
        var pic = [];
        pic.push(ids[0][0].files[0]);
        pic.push(ids[1][0].files[0]);
    ...
    //上传单张头像图片
    uploadSingleImg: function uploadSingleImg(_this, ids) {
        var that = this;
        var pic = [];
        pic.push(ids[0][0].files[0]);
        var fd = new FormData();
    ...
    //新建电站
    insertPlant: function insertPlant() {
        $('#npInfo').validate();
        App.initValidate();
        var that = this;
        that.getAllType();
        ...
        $('.orgDiv input').off('focus').on('focus', function () {
            var orgTree = App.dialog({
                ...
                closeEvent: null,
                isdrag: true,
                buttons: [{ text: $.getI18n('cancel'), type: 'imgNoBtn', clickToClose: true, id: 'cancelOwner' }, { text: $.getI18n('save'), type: 'imgNoBtn', clickToClose: true, id: 'saveOwner', click: function click() {
                        $('input[name="org"]').val(that.org);
        ...
        $('#getAddr').unbind().on('click', function () {
            var address = App.dialog({
                ...
                closeEvent: null,
                isdrag: true,
                buttons: [{ text: $.getI18n('cancel'), type: 'imgNoBtn', clickToClose: true, id: 'cancelOwner' }, { text: $.getI18n('sure'), type: 'imgNoBtn', clickToClose: true, id: 'saveOwner', click: function click() {
        ...
        //查看电价规则
        $('#seePrice').unbind().on('click', function () {
            var priceVal = $('#npInfo select[name="plantPrice"]').val();
            var priceInfos = App.dialog({
                title: '电价详情',
                width: 500,
                height: 500,
    ...
    //电价类型
    getPriceType: function getPriceType(obj) {
        var oa = '';
        var ob = '';
        ...
        if (obj.plantType == 1) {
            ...
            ob = '<option value="kWh">kWh</option>' + '<option value="gWh">gWh</option>' + '<option value="gWh">mWh</option>';
        }
    ...
    //获取腾讯经纬度
    getTxLatLng: function getTxLatLng(country, province, city) {
        var that = this;
