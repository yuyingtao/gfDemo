...
var OrganizationRender = {
    ...
    Render: function Render() {
        ...
        $('#modifyPassword').unbind().on('click', function () {
            ...
            var pd = '';
            var pwdContent = '<form id="pwdForm1">' + '<div class="form-group1">' + '<b>*' + '</b>' + '<label>' + '<b style="visibility: hidden">占</b>' + $.getI18n("oldPwd") + ':' + '<b style="visibility: hidden">&nbsp;</b>' + '</label>' + '<input name="OldPassword" type="password" id="password" required/>' + '</div>' + '<div class="form-group1">' + '<b>*' + '</b>' + '<label>' + '<b style="visibility: hidden">占</b>' + $.getI18n("newPwd") + ':' + '<b style="visibility: hidden">&nbsp;</b>' + '</label>' + '<input name="NewPassword" type="password" class="passwordCheck" id="NewPassword" required/>' + '</div>' + '<div class="form-group1">' + '<b>*' + '</b>' + '<label>' + '<b style="visibility: hidden">&nbsp;</b>' + $.getI18n("confirmPwd") + ':' + '<b style="visibility: hidden">&nbsp;</b>' + '</label>' + '<input name="repNewPassword" type="password" class="eq1Password" id="repNewPassword"/>' + '</div>' + '</form>';
            pd = App.dialog({
        ...
        function addHoverDom(treeId, treeNode) {
            ...
            if (btn) btn.bind("click", function () {
                ...
                function fn() {
                    var newOrganization = "";
                    // 弹窗
                    var winContent = "<form id='addOrgForm'><div class='orEdit' id='addCP' style='display: none'>" + "<p><span class='messName'>" + $.getI18n('preOrg') + "</span><b style='margin-right: 8px'>:</b><input id='preOrg' value='" + PreName + "' disabled='true' type='text'></p>" + " <p><span class='messName'><span>*</span>" + $.getI18n('orgNames') + "</span><b style='margin-right: 8px'>:</b><input id='orgName' class='addOrgName' type='text'name='1' required/></p>" + "<p><span class='messName'><span>*</span>" + $.getI18n('administratorAccount') + "</span><b style='margin-right: 8px'>:</b><input id='loginId'  class='checkAddRepeatLoginId'type='text' name='2' required/></p>" + "<p><span class='messName'><span>*</span>" + $.getI18n('setPassWord') + "</span><b style='margin-right: 8px'>:</b><input id='password' type='password' class='passwordCheck' name='3' required/></p>" + "<p><span class='messName'><span>*</span>" + $.getI18n('confirmPwd') + "</span><b style='margin-right: 8px'>:</b><input id='rqpassword' type='password' class='eqPassword' name='4' required/></p>" + "<p><span class='messName'>" + $.getI18n('email') + "</span><b style='margin-right: 8px'>:</b><input  id='email' type='email' class='checkAddRepeatEmail' name='5'></p>" + "<p class='phoneNum'><span class='messName'>" + $.getI18n('userTel') + "</span><b style='margin-right: 8px'>:</b><input id='userTel' class='checkAddRepeatTel mobile' type='text'/></p>" + "<p><span class='messName'>" + $.getI18n('orgStatus') + "</span><b style='margin-right: 8px'>:</b><span class='orImg'><img class='addIsOpen'  src='images/setting/enabled.png'></span></p>" + "</div>" + "<div class='orEdit'id='addBM' style='display: none'>" + "<p><span class='messName'>" + $.getI18n('preOrg') + "</span><b style='margin-right: 8px'>:</b><input id='preOrg' value='" + PreName + "' disabled='true' type='text'></p>" + " <p><span class='messName'><span>*</span>" + $.getI18n('orgNames') + "</span><b style='margin-right: 8px'>:</b><input id='BorgName' type='text' required/></p>" + "<p><span class='messName'>" + $.getI18n('orgStatus') + "</span><b style='margin-right: 8px'>:</b><span class='orImg'><img class='addIsOpen'  src='images/setting/enabled.png'></span></p>" + "</div>";
                    "</form>";
            ...
            if (btn1) btn1.bind("click", function () {
                ...
                function deleteOrg() {
                    $.http.POST('/org/deleteOrgInfoByUser.do', { tokenId: Cookies.getCook('tokenId'), orgId: treeNode.id }, function (result) {
                        _this.ztreeShow(_this);
                        App.warningDialog(result.msg);
                    });
                }
    ...
    getZtree: function getZtree(_this, id) {
        var that = this;
    ...
    //修改组织信息
    ModifyInfo: function ModifyInfo(orgId, userId, isOpen, _this) {
        $.http.POST('/org/updateOrgInfoByUser.do', {
            ...
            if (result.code == "100") {
                var setFontCss = function setFontCss(treeId, treeNode) {
                    return treeNode.level == 0 ? { color: "#333", "font-size": "20px" } : {};
                };
                ...
                var getFontCss = function getFontCss(treeId, treeNode) {
                    return !!treeNode.highlight ? { color: "#A60000", "font-weight": "bold" } : { color: "#333", "font-weight": "normal" };
                };
                //ztree模糊查询


                var updateNodes = function updateNodes(highlight, nodeList) {
                    var zTree = $.fn.zTree.getZTreeObj("ztree");
                    for (var i = 0, l = nodeList.length; i < l; i++) {
                        nodeList[i].highlight = highlight; //高亮显示搜索到的节点(highlight是自己设置的一个属性)
                        zTree.expandNode(nodeList[i].getParentNode(), true, false, false); //将搜索到的节点的父节点展开
                ...
                var addHoverDom = function addHoverDom(treeId, treeNode) {
                    // console.log(treeId, treeNode,treeNode.getParentNode().name,treeNode.name)
                    var Pname = "";
                    var PreName = "";
                    if (treeNode.getParentNode()) {
                ...
                var removeHoverDom = function removeHoverDom(treeId, treeNode) {
                    $("#diyBtn_" + treeNode.id).unbind().remove();
                    $("#diy1Btn_" + treeNode.id).unbind().remove();
                    $("#diyBtn_space_" + treeNode.id).unbind().remove();
                    $("#diy1Btn_space_" + treeNode.id).unbind().remove();
                ...
                var beforeClick = function beforeClick(treeId, treeNode, clickFlag) {
                    // console.log(treeNode)
                    if (treeNode.orgType == "0") {
                        $("#modifyPassword").show();
                    } else {
    ...
    deleteOrg: function deleteOrg(id) {
        $.http.POST('/org/deleteOrgInfoByUser.do', { tokenId: Cookies.getCook('tokenId'), orgId: id }, function (result) {
            App.warningDialog(result.body);
        });
    },
    //刷新当前节点
