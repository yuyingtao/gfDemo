...
var AccountManagementRender = {
        ...
        $("#delete").on("click", function () {
            ...
            $("#example tbody tr").each(function () {
                if ($(this).find("img").attr("src") == "images/repImages/rect1.png") {
                    var row = $("table#example tr").index($(this));
                    var cruId = _this.data[row - 1].id;
    ...
    ztreeShow: function ztreeShow(_this) {
        var treeNode;
    ...
    renderDataTable: function renderDataTable(_this, orgId, content, roleId, status) {
        if (_this.plantTable) {
        ...
        var columns = [{
            ...
            "bSortable": false
        }, { "data": "roleName", title: "" + $.getI18n('accountsRole') }, { "data": "userName", title: "" + $.getI18n('eachName'), render: function render(data, type, full, meta) {
                return '<div class="td-noWrap" title="' + data + '">' + data + '</div>';
            } }, { "data": "orgName", title: "" + $.getI18n('orgName'), render: function render(data, type, full, meta) {
                return '<div class="td-noWrap" title="' + data + '">' + data + '</div>';
            } }, { "data": "loginId", title: "" + $.getI18n('loginId'), render: function render(data, type, full, meta) {
                return '<div class="td-noWrap" title="' + data + '">' + data + '</div>';
            } }, { "data": "userTel", title: "" + $.getI18n('phoneNum'), render: function render(data, type, full, meta) {
                return '<div class="td-noWrap" title="' + data + '">' + data + '</div>';
            } }, { "data": "email", title: "" + $.getI18n('eachEmail'), render: function render(data, type, full, meta) {
                return '<div class="td-noWrap" title="' + data + '">' + data + '</div>';
            } }, {
            "data": "status",
        ...
        if (!!window.systemRole['accountManageEdit']) {
            columns.push({
                "data": "id", 'title': "" + $.getI18n('operate'), render: function render(data, type, full, meta) {
        ...
        _this.plantTable = $('#example').dataTable({
            ajax: {
                ...
                data: function data(d) {
                    d.tokenId = Cookies.getCook('tokenId'), d.content = content;
                    d.length = 10;
            ...
            columns: columns,
            columnDefs: [{ "width": "5%", "targets": 0 }, { "width": "10%", "targets": 1 }, { "width": "10%", "targets": 2 }, { "width": "10%", "targets": 3 }, { "width": "10%", "targets": 4 }, { "width": "15%", "targets": 5 }, { "width": "10%", "targets": 6 }, { "width": "5%", "targets": 7 }, { "width": "150px", "targets": 8 }],
        ...
        $("#example tbody").on("click", "#modify", function (event) {
            ...
            var cruId = _this.data[row - 1].id;
            var curOrgId = _this.data[row - 1].orgId;
            var curStatus = _this.data[row - 1].status;
            var iconUrl = _this.data[row - 1].icon;
            var roleName = _this.data[row - 1].roleName;
            var userName = _this.data[row - 1].userName;
            var curRoleId = void 0;
            // console.log(cruId)
            //弹出修改界面
        ...
        $("#example tbody").on("click", "#modifyPassword", function (event) {
            ...
            //获取行 row从1开始的 row-1 就是当前行
            var row = $("table#example tr").index($(this).closest("tr"));
            var userId = _this.data[row - 1].id;
            // console.log(_this.data[row-1])
            var pwdContent = '<form id="pwdForm1">' + '<div class="form-group1">' + '<b>*' + '</b>' + '<label>' + $.getI18n('newPwd') + '</label>' + '<input name="NewPassword" type="password" class="passwordCheck" id="NewPassword" required/>' + '</div>' + '<div class="form-group1">' + '<b>*' + '</b>' + '<label>' + $.getI18n('confirmPwd') + '</label>' + '<input name="repNewPassword" type="password" class="eq1Password" id="repNewPassword"/>' + '</div>' + '</form>';
            pd = App.dialog({
    ...
    //新增修改用户信息弹窗模板
    addUserTep: function addUserTep() {
        //弹窗模板
        this.ownerContent = '<form id="addUserForm" enctype="multipart/form-data">' + '<div class="leftInfo fl">' + '<div class="item-group">' + '<b>*' + '</b>' + "<label><span>" + $.getI18n("orgName") + "</span>：" + '</label>' + '<input name="orgName" id="addOrgName" disabled="true" required/>' + '</div>' + '<div class="item-group">' + '<b>*' + '</b>' + "<label><span>" + $.getI18n("userName") + "</span>：" + '</label>' + '<input name="roleName" id="addUserName" required/>' + '</div>' + '<div class="item-group">' + '<b>*' + '</b>' + "<label><span>" + $.getI18n("role") + "</span>：" + '</label>' + '<select name="role" id="addUserOption">' + '<option value="0">' + $.getI18n("all") + '</option>' + '</select>' + '</div>' + '<div class="item-group">' + '<b>*' + '</b>' + "<label><span>" + $.getI18n("userTel") + "</span>：" + '</label>' + '<input name="userTel" id="addUserTel" class=" checkAddRepeatTel mobile" required/>' + '</div>' + '<div class="item-group">' + '<b style="visibility: hidden">*' + '</b>' + "<label><span>" + $.getI18n("email") + "</span>：" + '</label>' + '<input name="email" id="addUserEmail" class="checkAddRepeatEmail" type="email"/>' + '</div>' + '</div>' + '<div class="rightInfo fr">' + '<div class="item-group">' + '<b>*' + '</b>' + '<label><span>' + $.getI18n("sysId") + '</span>：' + '</label>' + '<input name="loginId" placeholder="" id="addUserLoginId" class="checkAddRepeatLoginId sysTel"required/>' + '<span class="sysSpan"><span><img class="useTle"  id="addUserUseTel" src="/images/setting/noUseTel.png" alt=""></span><span>' + $.getI18n("useTel") + '</span></span>' + '</div>' + '<div class="item-group">' + '<b>*' + '</b>' + '<label><span>' + $.getI18n("accPassword") + '</span>：' + '</label>' + '<input name="password" type="password" id="password"  required class="passwordCheck firPwd"/>' + '</div>' + '<div class="item-group">' + '<b>*' + '</b>' + '<label><span>' + $.getI18n("confirmPwd") + '</span>：' + '</label>' + '<input type="password" required id="addUserQrpassword" class="eqPassword secPwd"/>' + '</div>' + '<div class="item-group orgStatus">' + '<b style="visibility: hidden">*' + '</b>' + '<label><span>' + $.getI18n("accountStatus") + '</span>：' + '</label>' + '<i><image id="addUserStatus" src="/images/plantManage/enabled.png" class="statusImg"></image></i>' + '</div>' + '<div class="item-group userPic">' + '<b style="visibility: hidden">*' + '</b>' + '<label style="position: absolute; top: 0;left: 10px"><span style="color: #333;">' + $.getI18n("userIcon") + '</span>：' + '</label>' + '<i style="position: absolute;left: 100px;display: inline-block"><div class="uploadImgBox"><image id="addUploadImgBox" src="/images/setting/upload.png"></image><a class="uploadA" id="addClose" style="display: none"><img src="/images/setting/close.png" alt=""></a><input id = "addUp_img" type="file" accept="image/jpeg,image/gif,image/png" /></div><span style="position:absolute;bottom:0"><span>' + $.getI18n("ProposedPictureLengthWidthRatio") + '</span>1:1</span></i>' + '</div>' + '</div>' + '</form>';
        return this.ownerContent;
    },
    //修改模板
    modifyUserTep: function modifyUserTep() {
        //弹窗模板
        this.ownerContent = '<form id="modifyForm" enctype="multipart/form-data">' + '<div class="leftInfo fl">' + '<div class="item-group">' + '<b>*' + '</b>' + "<label><span>" + $.getI18n("orgName") + "</span>：" + '</label>' + '<input name="orgName" id="modifyOrgInput" required/>' + '</div>' + '<div class="item-group">' + '<b>*' + '</b>' + "<label><span>" + $.getI18n("userName") + "</span>：" + '</label>' + '<input name="userName" id="modifyUserName" required/>' + '</div>' + '<div class="item-group">' + '<b>*' + '</b>' + "<label><span>" + $.getI18n("role") + "</span>：" + '</label>' + '<select name="role"  id="modifyOption">' + '</select>' + '</div>' + '<div class="item-group">' + '<b>*' + '</b>' + "<label><span>" + $.getI18n("userTel") + "</span>：" + '</label>' + '<input name="userTel" id="userTel" class=" checkRepeatTel mobile" required/>' + '</div>' + '<div class="item-group">' + '<b style="visibility: hidden">*' + '</b>' + "<label><span>" + $.getI18n("email") + "</span>：" + '</label>' + '<input name="email" id="userEmail" class="checkRepeatEmail" type="email"/>' + '</div>' + '</div>' + '<div class="rightInfo fr">' + '<div class="item-group">' + '<b>*' + '</b>' + '<label><span>' + $.getI18n("sysId") + '</span>：' + '</label>' + '<input name="loginId" placeholder="" id="userLoginId" class="checkRepeatLoginId sysTel" required/>' + '<span class="sysSpan"><span><img class="useTle"  id="modifyIsUseTel" src="/images/setting/noUseTel.png" alt=""></span><span>' + $.getI18n("useTel") + '</span></span>' + '</div>' + '<div class="item-group orgStatus">' + '<b style="visibility: hidden">*' + '</b>' + '<label><span>' + $.getI18n("accountStatus") + '</span>：' + '</label>' + '<i><image  src="" class="statusImg"></image></i>' + '</div>' + '<div class="item-group userPic">' + '<b>' + '</b>' + '<label style="position: absolute; top: 0;left: 10px"><span style="color: #333;">' + $.getI18n("userIcon") + '</span>：' + '</label>' + '<i style="position: absolute;left: 100px;"><div class="uploadImgBox"><image id="modifyUploadImgBox" src="/images/plantManage/head.png"></image><a class="uploadA">' + '<img src="/images/setting/close.png" alt=""></a>' + '<input id = "modifyUp_img" type="file" accept="image/jpeg,image/gif,image/png"/>' + '</div><span style="position:absolute;bottom:0;color: #FF0E26"><span>' + $.getI18n("ProposedPictureLengthWidthRatio") + '</span>1:1</span></span></i>' + '</div>' + '</div>' + '</form>';
        return this.ownerContent;
    },
    //修改信息选择组织弹窗
    modifyChooseOrgTree: function modifyChooseOrgTree() {
        this.modifyChooseOrgTreeTem = "\n        <div id=\"modifyChooseOrgTree\" class=\"ztree\"></div>\n        ";
        return this.modifyChooseOrgTreeTem;
    },
    // 删除用户模板
    deleteUserTem: function deleteUserTem() {
        this.ownerContent = "" + $.getI18n('sureDeleteUser');
        return this.ownerContent;
    },
    //获取角色类型
    getRoleByOrgId: function getRoleByOrgId(orgId) {
    ...
    //图片上传
    uploadImg: function uploadImg(_this, ids, userId, iconUrl) {
        var pic = [];
        ...
        $.ajax({
            ...
            contentType: false,
            processData: false,
            success: function success(result) {}, complete: function complete() {
                _this.plantTable._fnDraw();
                var file = ids;
